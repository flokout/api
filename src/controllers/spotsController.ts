...
export class SpotsController {
   ...
   */
  static async getSpots(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
   ...
   * Create new spot
   */
  static async createSpot(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
   ...
   * Get spot by ID
   */
  static async getSpotById(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
   ...
   * Update spot
   */
  static async updateSpot(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
      ...
      const { data: currentSpot, error: getError } = await supabaseClient
        .from('spots')
      ...
      const { data: spot, error: updateError } = await supabaseClient
        .from('spots')
        .update({
          name,
          address,
   ...
   * Delete spot
   */
  static async deleteSpot(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
   ...
   */
  static async searchSpots(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
      ...
      if (flok_id) {
        searchQuery = searchQuery.eq('flok_id', flok_id as string);
      }
   ...
   */
  static async getSpotsByFlok(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;

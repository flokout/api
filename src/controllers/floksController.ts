...
export class FloksController {
  /**
   * Get user's floks
   */
  static async getUserFloks(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
      ...
      const { data: flokmates, error: flokError } = await supabaseClient
        .from('flokmates')
        .select(`
          flok_id,
          role,
      ...
      const floksWithMemberCount = await Promise.all(
        (flokmates || []).map(async (fm) => {
   ...
   * Create new flok
   */
  static async createFlok(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
      ...
      // Create flok
      const { data: flok, error: flokError } = await supabaseClient
        .from('floks')
        .insert({
          name,
   ...
   * Get flok by ID
   */
  static async getFlokById(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
      ...
      const { data: flokmates, error: flokmatesError } = await supabaseClient
        .from('flokmates')
        .select(`
          id,
          role,
   ...
   * Update flok
   */
  static async updateFlok(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
      ...
      const { data: flok, error: updateError } = await supabaseClient
        .from('floks')
        .update({ name })
        .eq('id', id)
        .select()
   ...
   * Delete flok
   */
  static async deleteFlok(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
   ...
   * Get flok members
   */
  static async getFlokMembers(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
   ...
   */
  static async joinFlok(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
   ...
   * Leave flok
   */
  static async leaveFlok(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
   ...
   * Get flok invites
   */
  static async getFlokInvites(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
   ...
   * Delete invite
   */
  static async deleteInvite(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
      ...
      const { data: invite, error: getError } = await supabaseClient
        .from('flok_invites')
        .select('flok_id')
        .eq('id', inviteId)
        .single();
   ...
   */
  static async verifyInvite(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
      ...
      const { data: flokData, error: flokError } = await supabaseClient
        .from('floks')
        .select(`
          id,
          name,
   ...
   */
  static async associateSpot(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
   ...
   */
  static async disassociateSpot(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
   ...
   * Reactivate flok
   */
  static async reactivateFlok(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
   ...
   */
  static async purgeFlok(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Not authenticated' });
        return;
      ...
      const { data: flokoutIds } = await supabaseClient
        .from('flokouts')
        .select('id')
        .eq('flok_id', id);
      ...
      // 2. Delete flokouts
      const { error: flokoutsError } = await supabaseClient
        .from('flokouts')
        .delete()
        .eq('flok_id', id);
      ...
      const { error: flokSpotsError } = await supabaseClient
        .from('flok_spots')
        .delete()
        .eq('flok_id', id);
      ...
      // 4. Delete invites
      const { error: invitesError } = await supabaseClient
        .from('flok_invites')
        .delete()
        .eq('flok_id', id);
      ...
      const { error: flokmatesError } = await supabaseClient
        .from('flokmates')
        .delete()
        .eq('flok_id', id);
      ...
      const { error: flokDeleteError } = await supabaseClient
        .from('floks')
        .delete()
        .eq('id', id);
